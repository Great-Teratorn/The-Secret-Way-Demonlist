...
            if ((this.gameMode === 'survival' || this.gameMode === 'secret-progression') && this.levels.length > 0) {                try {                    const response = await fetch(`data/${this.levels[0].path}.json`);                    if (response.ok) {                        const firstLevelData = await response.json();                        this.survivalTarget = firstLevelData.secret_way_at || 1;                    } else {                        this.survivalTarget = 1;                    }                    this.percentage = undefined;                } catch (err) {                    this.survivalTarget = 1;                    this.percentage = undefined;                }            } else {                this.percentage = undefined;            }
