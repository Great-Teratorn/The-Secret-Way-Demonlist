...
methods: {        shuffle,        getThumbnailFromId,        getYoutubeIdFromUrl,        async onStart() {            // 1. Force clear state tracking fields to guarantee a full, clean initialization            this.showRemaining = false;            this.givenUp = false;            this.progression = [];            this.levels = [];
...
            // 2. Strict Mode Sorting Controller            if (this.gameMode === 'linear' || this.gameMode === 'secret-progression') {                if (this.progressionOrder === 'ascending') {                    // Easiest to Hardest: Higher numerical rank to lower rank (e.g. #150 down to #1)                    this.levels = safeListCopy.sort((a, b) => b.rank - a.rank);                } else {                    // Descending (Hardest to Easiest): Lower rank up to higher numerical rank (e.g. #1 up to #150)                    this.levels = safeListCopy.sort((a, b) => a.rank - b.rank);                }            } else {                // Classic Random & Secret Way Survival remain completely randomized                this.levels = shuffle([...safeListCopy]).slice(0, 100);            }
            // 3. Setup Secret Way Target Meta Data            if (this.gameMode === 'survival' || this.gameMode === 'secret-progression') {                await Promise.all(                    this.levels.map(async (lvl) => {                        try {                            let lvlName = lvl.name || '';                            lvlName = lvlName.toLowerCase().replace(/\s+/g, '-');                            const res = await fetch(`data/${lvlName}.json`);                            if (res.ok) {                                const data = await res.json();                                lvl.secret_way_at = data.secret_way_at || 1;                            } else {                                lvl.secret_way_at = 1;                            }                        } catch (e) {                            lvl.secret_way_at = 1;                        }                    })                );            }
