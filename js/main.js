...
// GLOBAL CROSS-PAGE LIST SEARCH ARCHITECTUREfunction executeToggleSearch() {    const searchBox = document.getElementById('welcomeSearchBox');    const input = document.getElementById('levelSearch');    if (!searchBox || !input) return;        searchBox.classList.toggle('active');        if (searchBox.classList.contains('active')) {        input.focus();    } else {        input.value = '';    }}
...
document.addEventListener('click', function(event) {    if (event.target.closest('#welcomeSearchBtn')) {        event.preventDefault();        executeToggleSearch();    }});
document.addEventListener('keydown', function(event) {    if (event.target && event.target.id === 'levelSearch' && event.key === 'Enter') {        const query = event.target.value.trim();        if (query.length > 0) {            localStorage.setItem('pendingListSearch', query);            window.location.hash = '/list';        }    }});
